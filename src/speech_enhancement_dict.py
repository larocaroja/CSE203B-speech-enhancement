...
class SpeechEnhancement:
    ...
    def dictionary_learning(self,B):
        ...
        """
        self.dict_learning = DictionaryLearning(n_components=self.config.n_atoms, alpha=self.config.alpha_val, max_iter=self.config.num_lasso_iterations, transform_max_iter = self.config.num_lasso_iterations, fit_algorithm='cd', transform_algorithm='lasso_lars', positive_code = True, positive_dict = True, n_jobs=-1, tol = 1e-4, verbose = True)
        H = self.dict_learning.fit_transform(B)
        print(H.sum(axis=1))
        # dict_learning.fit(B)

        return self.dict_learning.components_ # (n_components, n_features)
    ...
    def denoise_signal(self, noisy_signal):
        ...
        sparse_code = self.dict_learning.transform(noisy_mel_spectrogram) # (n_samples, n_components)
        denoised_signal = np.dot(sparse_code, self.D)
        # print(denoised_signal.shape)
        self.denoised_mel_spectrogram = denoised_signal.reshape(b, -1, f)

        return self.denoised_mel_spectrogram, self.phase_noisy
    ...
    def inverse_mel_spectrogram(self, mel_spectrogram, phase):
        ...
        stft = stft * phase
        print(stft.shape)
        waveform = librosa.istft(stft, n_fft = self.config.n_fft , hop_length=self.config.hop_size)
    ...
    def visualize_spectrogram(self, clean_speech, noisy_speech):
        ...
        for i in range(clean_speech.shape[0]):
            ...
            mel_spectrogram_denoised = self.denoise_signal([noisy_speech[i]])[0][0].T
            # print(mel_spectrogram_denoised.shape)
        ...
        plt.suptitle(f"Noise type: {self.config.noise_type}, SNR: {self.config.SNR_dB} dB")
        plt.tight_layout()
        plt.savefig(f'mel_spectrogram_{self.config.n_atoms}_{self.config.alpha_val}_{self.config.noise_type}_{self.config.SNR_dB}.png')
        plt.show()
    ...
    def evaluate(self, ref, deg):
        ...
        """
        assert len(ref) == len(deg), f"Number of clean speech samples ({len(clean_speech_val)}) does not match the number of denoised samples ({len(self.denoised_mel_spectrogram)})."
        ...
        # print(clean_speech_val.shape, denoised_signal_inv.shape)
        # print(clean_speech_val.mean(axis=1)[:10], denoised_signal_inv.mean(axis=1)[:10])
        pesq_dict = defaultdict(list)    

        for i in range(len(ref)):
            pesq_score_wb = pesq.pesq(fs = self.config.sample_rate, ref = ref[i], deg = deg[i], mode = 'wb')
            pesq_score_nb = pesq.pesq(fs = self.config.sample_rate, ref = ref[i], deg = deg[i], mode = 'nb')
...
def load_Librispeech_data(dataset_dir, train = True, dataset_size = -1):
    """
    ...
    for file in dataset_files[:dataset_size]:
...
if __name__ == "__main__":
    ...
    clean_speech_train = load_Librispeech_data(config.dataset_dir, train = True, dataset_size = config.train_dataset_size)
    clean_speech_val = load_Librispeech_data(config.dataset_dir, train = False, dataset_size = config.valid_dataset_size)
    ...
    mel_spectrogram_denoised, phase_noisy = speech_enh.denoise_signal(noisy_speech_val)
    denoised_speech_val = np.array([speech_enh.inverse_mel_spectrogram(mel_spectrogram, phase) for mel_spectrogram, phase in zip(mel_spectrogram_denoised, phase_noisy)])
    # print(mel_spectrogram_denoised.shape)
    # print(denoised_speech_val.shape)
    ...
    print("Denoised signal (first sample):", mel_spectrogram_denoised[0])

    try:
        # Evaluate the denoised signal using PESQ
        pesq_dict = speech_enh.evaluate(clean_speech_val, noisy_speech_val)
        print(f"Clean vs. Noisy")
        print(f"PESQ score (wideband): {np.mean(pesq_dict['wb'])}")
        print(f"PESQ score (narrowband): {np.mean(pesq_dict['nb'])}")

        pesq_dict= speech_enh.evaluate(clean_speech_val, denoised_speech_val)
        print(f"Clean vs. Denoised")
        print(f"PESQ score (wideband): {np.mean(pesq_dict['wb'])}")
        print(f"PESQ score (narrowband): {np.mean(pesq_dict['nb'])}")
